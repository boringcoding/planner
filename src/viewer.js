...
(() => {
  ...
  window.startViewer = async function (root) {
    ...
    try {
      ...
      // Подсказка по тому, чем управляют на самом деле. На телефоне нет ни
      // правой кнопки, ни колеса, и строка про них — не подсказка, а тупик:
      // сдвиг двумя пальцами работает, но узнать о нём неоткуда
      const touch = matchMedia('(pointer: coarse)').matches;
      const baseline = () => `${(buf.byteLength / 1024).toFixed(0)} КБ · ${tri.toLocaleString('ru')} треугольников · `
        + (touch
          ? `палец — поворот, два пальца — сдвиг и приближение · тап — что это, двойной тап — камера вокруг него`
          : `тяните — поворот, правой кнопкой или Shift — сдвиг, колесо — приблизить · клик — что это, двойной клик — камера вокруг него`);

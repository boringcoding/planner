...
const CASES = [
  ...
  ['времянку придвинули к дому', /отступление не оправдывает/,
    h => { h.project.plot.temp.y -= 3000; }],

  ['у времянки отняли запись об отступлении', /отступление по разрыву не записано/,
    h => { delete h.project.plot.temp.waiver; }],

  ['времянку сделали постоянным домом', /противопожарный разрыв/,
    h => { delete h.project.plot.temp.role; }],

  ['помещение времянки съехало с перегородки', /не принадлежит ни одному помещению/,
    h => { h.project.plot.temp.rooms[0].x += 200; }],

  ['окно времянки уехало в угол', /не оставляет простенка/,
    h => { const w = h.project.plot.temp.windows.find(q => q.id === 'plot.tg4'); w.a = 3150; w.b = 4550; }],

  ['витраж времянки перерос скат', /низ кровли над ним/,
    h => { h.project.plot.temp.windows.find(q => q.id === 'plot.tg2').hz = 2900; }],

  ['во времянке заложили дверь в спальню', /не войти от входной двери/,
    h => { h.project.plot.temp.doors = h.project.plot.temp.doors.filter(q => q.id !== 'plot.tv3'); }],

  ['потолок времянки опустили под скат', /под скатом/,
    h => { h.project.plot.temp.clear = 1900; }],

  ['свес времянки вылез за границу участка', /свес кровли вышел за границу/,
    h => { h.project.plot.temp.roof.over = 3200; }],

  ['кровля времянки не накрыла настил', /не перекрывает настил/,
    h => { h.project.plot.temp.deck.in = -700; }],

  ['свая времянки не дошла до промерзания', /низ сваи/,
    h => { h.project.plot.temp.pile.depth = 1000; }],

  ['ростверк времянки лёг на землю', /продух под ростверком/,
    h => { h.project.plot.temp.pile.drop = 900; }],

  ['ограждение настила времянки осталось без тел', /ограждение настила задано, а тел у него нет/,
    h => { const D = h.project.plot.temp.deck; D.steps.a = 54; D.steps.b = 8056; }],

  ['настил времянки опустили к земле', /подступёнок крыльца/,
    h => { h.project.plot.temp.floor = -200; }],

  ['проступь крыльца времянки укоротили', /проступь крыльца/,
    h => { h.project.plot.temp.deck.steps.tread = 200; }],

  ['порог времянки стал ступенью', /это ступень, а не отбойник/,
    h => { h.project.plot.temp.deck.drop = 400; }],

  ['ограждение времянки занизили в решении', /ограждение 500 при норме/,
    h => { h.project.plot.temp.deck.rail = 500; }],

  ['идентификатор во времянке повторился', /идентификатор plot.tr1 повторяется/,
    h => { h.project.plot.temp.rooms[1].id = 'plot.tr1'; }],

  ['помещение времянки вылезло за стену', /выходит за внутренний габарит/,
    h => { h.project.plot.temp.rooms[4].h = 2800; }],

  ['перегородки времянки наложились', /наложились/,
    h => { h.project.plot.temp.parts[3].w = 2400; }],

  ['окна времянки сошлись вплотную', /разошлись на/,
    h => { h.project.plot.temp.windows.find(q => q.id === 'plot.tg2').a = 5420; }],

  ['дверь времянки сузили до щели', /пройти можно от/,
    h => { h.project.plot.temp.doors[0].b = h.project.plot.temp.doors[0].a + 400; }],

  ['дверь времянки занизили', /пройти не пригибаясь/,
    h => { h.project.plot.temp.doors[0].hz = 1200; }],

  ['вход времянки развернули на глухую стену', /не ведёт ни в одно помещение/,
    h => { const T = h.project.plot.temp; T.door.side = 'W'; T.door.a = 500; T.door.b = 1500; }],

  ['уклон кровли времянки положили', /уклон кровли/,
    h => { h.project.plot.temp.roof.pitch = 8; }],

  ['свес кровли времянки срезали', /свес кровли \d+, норма/,
    h => { h.project.plot.temp.roof.over = 200; }],
  ...
  ['септик уполз в глубину участка', /обслуживать с улицы/,
    h => { h.project.plot.septic.y = 12600; }],
...
const SCASES = [
  ...
  ['кабель лёг вплотную к воде', /идут параллельно/,
    d => { sys(d, 'eom').feeds.find(f => f.id === 'eom.in2').enter.at = 23300; }],
  ...
  ['ввод во времянку сел на террасу', /а стена блока идёт/,
    d => { sys(d, 'vk').feeds.find(f => f.id === 'vk.in2').enter.at = 18000; }],

...
// Строка про времянку собирается из геометрии, а не из констант. Раньше
// здесь стояло «из блока в дальнем юго-восточном углу … два несгораемых —
// норма 6»: коробку заменили каркасным домом, а страница продолжала это
// печатать — и противоречила допущениям на том же листе. Материал, норма,
// разрыв и сам факт отступления теперь берутся из данных
const tempRow = T => {
  ...
  const gap = dist({ x: 0, y: 0, w: house.shell.w, h: house.shell.h }, T.box);
  const wood = T.mat === 'wood';
  const need = wood ? 10 : 6;
  return `${m1(T.w)} × ${m1(T.h)} м, ${wood ? 'каркасный дом' : 'блочный дом'} на ${T.piles.length} сваях`
    + `, терраса ${m1(T.deck.w)} × ${m1(T.deck.h)} под общей кровлей ${T.roof.pitch}°, конёк ${(T.top / 1000).toFixed(3).replace('.', ',')}`
    + ` — жильё на время стройки; помещений ${T.rooms.length}, ${T.area.toFixed(1).replace('.', ',')} м²`
    + `; противопожарный разрыв до дома ${m1(gap)} м при норме ${need} (${wood ? 'камень — дерево' : 'два несгораемых'})`
    + (T.role === 'temporary' ? ': отступление, строение временное' : '');
};
...
const plotSection = (() => {
  ...
  const rows = [
    ...
    ...(plotG.temp ? [['Времянка', tempRow(plotG.temp)]] : []),
    ['Забор', `${(plotG.fence.len / 1000).toFixed(0)} м по периметру, въездные ворота ${plotG.fence.gate ? (plotG.fence.gate.w / 1000).toFixed(1).replace('.', ',') : '—'} м по оси гаражного фронта, калитка против дорожки`],
